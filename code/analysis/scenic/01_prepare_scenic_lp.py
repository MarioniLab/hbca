...
output_dir = '/nfs/research/marioni/areed/projects/hbca/scenic/2022-04-05/scvi_new/lp/output'
...
celltypes_to_study = ['LP1', 'LP2', 'LP3', 'LP4', 'LP proliferating', 'Other epithelial (1)', 'Other epithelial (2)']
adata = adata[adata.obs.level2.isin(celltypes_to_study), :]

# save full loom
f_loom_path_scenic = output_dir + "/loom/loom_lp_all.loom"
...
f_loom_path_scenic = output_dir + "/loom/loom_lp_1k.loom"
...
#save a subsetted version (n=4*10000 + #(LP proliferating, Other epithelial (1), Other epithelial (2)) )
np.random.seed(seed=42)
...
for celltype_select in celltypes_to_study:
    ...
    cells_selected = cells_of_type[np.random.choice(np.arange(0, len(cells_of_type)),
                                                    size=min(10000, len(cells_of_type)), replace=False)]
...
# create basic row and column attributes for the loom file:
f_loom_path_scenic = output_dir + "/loom/loom_lp_40k.loom"
row_attrs = {"Gene": np.array(adata_sub.var_names)}
col_attrs = {
    "CellID": np.array(adata_sub.obs_names),
    "nGene": np.array(np.sum(adata_sub.X.transpose() > 0, axis=0)).flatten(),
...
for celltype_select in celltypes_to_study:
    print(celltype_select)
    if celltype_select in ['LP1', 'LP2', 'LP3', 'LP4']:
...
adata_sub = adata[cells_selected_total]
# create basic row and column attributes for the loom file:
f_loom_path_scenic = output_dir + "/loom/loom_lp_40k_2.loom"
row_attrs = {"Gene": np.array(adata_sub.var_names)}
col_attrs = {
    "CellID": np.array(adata_sub.obs_names),
