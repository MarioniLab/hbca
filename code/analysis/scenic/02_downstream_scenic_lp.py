...
#load data
subset_depth = '40k_2' #'40k' #'1k'
f_final_loom = '../lp/output/scenic_out/SCENIC_OUT_subset' + subset_depth +'.loom'
...
figdir = '/nfs/research/marioni/areed/projects/hbca/scenic/2022-04-05/scvi_new/lp/output/plots/'
...
plt.tight_layout()
os.makedirs('/nfs/research/marioni/areed/projects/hbca/scenic/2022-04-05/scvi_new/lp/output/plots/regulons/', exist_ok=True)
...
ax4.set_ylabel('')
plot_rss(rss_cellType, 'LP5', ax=ax5)
ax5.set_ylabel('')
plot_rss(rss_cellType, 'DDC1', ax=ax6)
ax6.set_ylabel('')
plot_rss(rss_cellType, 'DDC2', ax=ax7)
plt.tight_layout()
...
#colors = sns.color_palette(['#DDA0DD', '#F4C4F4', '#C98EE8', '#B578B5', '#FC8FFC', '#DDDDDD']) #1k
colors = sns.color_palette(['#540F54', '#53407F', '#DA80DA', '#815481', '#C040C0', '#E1AFE1', '#3F0034']) #40k
colors2 = sns.color_palette(['#ff9d9a', '#f1ce63', '#4e79a7', '#a0cbe8', '#f28e2b', '#bab0ac']) #40k
...
#get ordered topreg
topreg = []
for i, c in enumerate(['LP1', 'LP2', 'LP3', 'LP4', 'LP5', 'DDC1', 'DDC2']):
    topreg.extend(list(rss_cellType.T[c].sort_values(ascending=False)[:10].index))
topreg = list(set(topreg))
...
g_nocluster_rast = sns.clustermap(auc_mtx_Z[topreg].reset_index(drop=True).T, annot=False, square=False, linecolor='gray',
                                 xticklabels=False, yticklabels=True, vmin=-2, vmax=4, col_colors=colormap,
                                 cmap="YlGnBu",
                                 figsize=(21, 16), col_cluster=False, rasterized=True)
...
##Tissue_condition
if True:
    g_nocluster = sns.clustermap(auc_mtx_Z[topreg2].reset_index(drop=True).T, annot=False, square=False, linecolor='gray',
                                 xticklabels=False, yticklabels=True, vmin=-2, vmax=4, col_colors=colormap,
                                 cmap="YlGnBu",
                                 figsize=(21, 16), col_cluster=False)
    g_nocluster.cax.set_visible(True)
    g_nocluster.ax_heatmap.set_ylabel('')
    g_nocluster.ax_heatmap.set_xlabel('')
    g_nocluster.savefig(figdir + 'heatmap/heatmap_tissueCond_nocluster' + subset_depth + '.png')

    g_nocluster.savefig(figdir + 'heatmap/heatmap_tissueCond_nocluster' + subset_depth + '.pdf')
