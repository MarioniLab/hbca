...
output_dir = '/nfs/research/marioni/areed/projects/hbca/scenic/2022-04-05/scvi_new/bsl/output'
...
celltypes_to_study = ['BSL1', 'BSL2']
adata = adata[adata.obs.level2.isin(celltypes_to_study), :]

# save full loom
f_loom_path_scenic = output_dir + "/loom/loom_bsl_all.loom"
...
f_loom_path_scenic = output_dir + "/loom/loom_bsl_1k.loom"
...
for celltype_select in celltypes_to_study:
    ...
    if celltype_select in ['BSL1', 'BSL2']:
...
# create basic row and column attributes for the loom file:
f_loom_path_scenic = output_dir + "/loom/loom_bsl_sub1.loom"
row_attrs = {"Gene": np.array(adata_sub.var_names)}
col_attrs = {
    "CellID": np.array(adata_sub.obs_names),
